...
When no `ai.quiz` chain entry has a matching environment provider key, quiz-definition generation does **not** fail closed. The same `QuizDefinitionGenerator` path falls back to an application-owned structural scaffold built from the sanitized brief (stable IDs, supported question types, opening copy, `result.mode=ai`). Brief fields remain untrusted plain text and are never treated as instructions. Administrator AI quiz creation in the panel uses only the **AI quiz interview** chat interface (header action on quiz create/edit and the AI quiz discovery page). There is no separate Generate AI draft brief form in the admin UI. Programmatic generation (`POST /api/v1/quizzes/generate` and `GenerateQuizDraft`) likewise returns a validated scaffold instead of `ai_unavailable` when credentials are missing. Missing quiz AI credentials therefore must not block draft creation for administrators or server-to-server agents. Report/analysis generation is unchanged: it still raises `GenerationException` with `ai_unavailable` when the report chain has no usable credentials. When a quiz chain has usable credentials but every provider attempt fails, quiz generation raises `ai_generation_failed` (not the credential-missing path).

### 12.1a AI quiz discovery interview

Before creating a quiz draft, an administrator opens **AI quiz interview** (create/edit header or `/admin/quiz-discovery`). The interview persists append-only user/assistant messages and a mutable, reviewed structured brief owned by the administrator. It asks one focused question at a time for missing business context, target audience, objective, and desired respondent insight; the administrator can edit the resulting brief directly. Each interviewer turn returns a schema-constrained result with `message`, allowlisted `brief` fields, and `action` of `continue`, `ready`, or `execute`. When the four core brief fields are complete, the agent sets `ready`, returns the structured brief, and invites review. When the administrator says to execute/generate/create now (or confirms Execute now on the brief), the agent sets `execute` only if the core brief is ready, finalizes the allowlisted brief structure, and the application runs `GenerateQuizDraft` from that brief — never from raw chat text. Control phrases are not written into brief fields. The interview uses the configured quiz AI provider/model chain when credentials are available and safely falls back to the same deterministic guided questions (including ready/execute detection) otherwise. Raw chat is untrusted reference material and is never supplied to `GenerateQuizDraft`; only the reviewed allowlisted brief is passed to generation. Each session snapshots the configurable discovery system prompt; generation retains its own immutable quiz-definition prompt/audit contract.
...
### 2026-08-27 — AI quiz interview is the only admin AI create path

- Removed the Generate AI draft brief-form header action from quiz create/edit. Administrators create AI drafts only through the AI quiz interview chat (header modal or AI quiz discovery page).
- Interview turns now return `continue` | `ready` | `execute` with a structured allowlisted brief. When the interview is complete or the administrator says execute/generate now, the agent finalizes the brief structure and the app generates the draft from that brief (not raw chat). Control phrases are never stored in brief fields.
- Server-to-server `POST /api/v1/quizzes/generate` remains the programmatic structured-brief path and still scaffolds when quiz AI credentials are missing.
