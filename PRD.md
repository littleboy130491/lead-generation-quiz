...
When no `ai.quiz` chain entry has a matching environment provider key, quiz-definition generation does **not** fail closed. The same `QuizDefinitionGenerator` path falls back to an application-owned structural scaffold built from the sanitized brief (stable IDs, supported question types, opening copy, `result.mode=ai`). Brief fields remain untrusted plain text and are never treated as instructions. The **AI quiz interview** header action on quiz create and edit remains available; when credentials are missing it still creates a validated structural scaffold from the reviewed brief after confirmation. Programmatic generation (`POST /api/v1/quizzes/generate` and `GenerateQuizDraft`) likewise returns a validated scaffold instead of `ai_unavailable`. Missing quiz AI credentials therefore must not block draft creation for administrators or server-to-server agents. Report/analysis generation is unchanged: it still raises `GenerationException` with `ai_unavailable` when the report chain has no usable credentials. When a quiz chain has usable credentials but every provider attempt fails, quiz generation raises `ai_generation_failed` (not the credential-missing path).

### 12.1a AI quiz discovery interview

Before creating a quiz draft, an administrator can open **AI quiz discovery** in the admin panel (standalone page or quiz create/edit header action). The interview persists append-only user/assistant messages and a mutable, reviewed structured brief owned by the administrator. It asks one focused question at a time for missing business context, target audience, objective, and desired respondent insight; the administrator can edit the resulting brief directly. The interview uses the configured quiz AI provider/model chain when credentials are available and safely falls back to the same deterministic guided questions otherwise. When the four core brief fields are complete, or when the administrator says **execute now** (or equivalent generate/create phrasing), the interviewer finalizes the allowlisted brief structure, sets `ready_to_generate`, opens brief review, and—on explicit execute phrasing—creates the draft immediately after review prerequisites are met. Raw chat is untrusted reference material and is never supplied to `GenerateQuizDraft`; only the reviewed allowlisted brief is passed to generation after explicit confirmation. Each session snapshots the configurable discovery system prompt; generation retains its own immutable quiz-definition prompt/audit contract. On quiz edit, generation updates the current quiz instead of creating a duplicate record.
...
- AI quiz interview action on quiz create and edit. The header action is always visible. When no `ai.quiz` chain entry has usable environment credentials, generation still succeeds with a validated structural scaffold from the reviewed brief after confirmation.
...
### 2026-08-27 — AI quiz interview is the sole admin draft-generation UI

- Removed the separate **Generate AI draft** brief-form header action from quiz create and edit. Administrators now create quiz drafts only through the **AI quiz interview** chat flow (modal on create/edit and standalone `/admin/quiz-discovery`).
- The discovery interviewer now returns a schema-constrained `ready_to_generate` flag plus the full allowlisted brief (`question_count` and `tone` included). When the core brief is complete or the administrator says **execute now** (or equivalent phrasing), the chat opens brief review and can immediately create the draft.
- Opening the interview from quiz edit now generates into the current quiz instead of creating a duplicate record.
