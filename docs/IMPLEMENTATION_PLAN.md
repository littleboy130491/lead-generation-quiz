...
Provide an authenticated Filament page and quiz create/edit modal that record a bounded interviewer conversation, derive only the allowlisted structured brief fields, and show them for optional administrator review/editing. The interview chat is the only administrator AI quiz-creation UI. Each turn is schema-constrained to a chat message, allowlisted brief fields, and `action` continue or generate; interviewer action `generate` means ready-to-offer, not permission to execute. Treat null, zero, and out-of-range question counts as unspecified instead of coercing them to one. Preserve valid explicit counts, while an omitted count is left for the quiz-generation agent to choose from the brief under the non-configurable generation contract. Recognize explicit generate-now intent locally before calling the interviewer. When the core brief is complete, move the session only to `ready`, keep the composer active, and expose **Create quiz now** while allowing further context turns. Never dispatch from completeness or a model-selected action alone. Only an explicit administrator button click or locally recognized execute/create/generate-now chat command may atomically associate the session with its target quiz, enter `generating`, and dispatch `GenerateQuizDraftJob` to the `ai` queue. Return that explicit generation request immediately, poll only while active, append assistant completion/failure messages, expose the generated draft link without forced redirect, and allow failed work to retry without duplicating an active attempt. Immediate generation needs at least business context or objective. Use the configured quiz chain when credentials exist; retain a deterministic guided fallback so discovery remains usable without credentials. Do not send the raw transcript to quiz generation. Do not emit quiz JSON in the chat message.

### 6.4 Admin generation flow

Run administrator quiz-draft generation on the `ai` queue and keep the Livewire interview responsive through persisted polling state. While a session is `generating`, provide a session-owner stop action. Cancellation must atomically transition the discovery session to `cancelled`, preserve its reviewed brief and append-only transcript, and allow a later retry as a new audit attempt. Jobs that have not started must no-op after cancellation; jobs already waiting on a provider must re-confirm and claim the same active execution inside the draft-persistence transaction so a cancelled session cannot receive a late draft, audit completion, or completion message. Record cancelled generation audits separately from failed attempts and test queued cancellation, in-flight result fencing, ownership, and retry.

Administrator generation is triggered only by an explicit execute/create/generate-now command or **Create quiz now**, then executed by an atomically claimed queued job; interviewer `action=generate` only exposes that choice. The server-to-server API remains synchronous and accepts the structured brief directly. Persist the session-to-quiz relationship before dispatch so polling and reopened chats resolve the same draft. When the persisted `ai.quiz` chain has no usable environment credentials, generation falls back to a validated structural scaffold from the sanitized brief. When credentials exist, use the provider chain with a V1 structured-output JSON schema matching `QuizDefinitionValidator` (schema_version, result, blocks, optional opening/score_results/thank_you); if every attempt fails, raise `ai_generation_failed`. Before validation, remove only leading, trailing, or consecutive generated page-break markers; preserve all content/question blocks and reject every other contract defect normally. Validate the resulting definition before applying it to the mutable draft. Exercise the job directly and prove duplicate deliveries cannot regenerate a completed session.
...
### 6.5 Contextual AI editing of an existing draft

On `EditQuiz`, replace the new-quiz interview header action with **Edit with AI**. Persist an edit-mode discovery session that snapshots the quiz name, description, and complete current `draft_definition`; delimit that snapshot as untrusted context on every interviewer turn and on final generation. Keep creation and editing sessions separate. The edit agent collects requested changes and recommends improvements while the existing draft remains untouched. Expose **Update quiz** only after the session is ready, and require that explicit action (or a locally recognized update-now command) before queueing work. Generate a complete V1 definition, validate it, then atomically replace only the mutable draft definition. Preserve quiz identity/settings and all published revisions. Reuse cancellation, retry, execution-token fencing, audit snapshots, and provider-chain behavior. Feature tests prove edit-only action copy, source-context delivery, explicit consent, full replacement, and published-revision immutability.

Render persisted assistant messages in both creation and editing chats as controlled CommonMark. Strip raw HTML and disable unsafe links, retain administrator messages as escaped plain text, and cover both formatting and injection behavior in the Livewire feature tests. Preserve whitespace on the escaped text element itself rather than the surrounding Blade container so template indentation cannot enlarge user bubbles while intentional message line breaks remain intact. Place the readiness-gated **Create quiz now** / **Update quiz** action directly after the transcript instead of in the chat header, and test that its rendered position follows the latest assistant message without changing execution authorization.

After a successful edit update, leave the composer active. On the first subsequent reply, create a linked edit session automatically, inherit the reviewed brief, snapshot the quiz's newly updated mutable definition, replay only a bounded linked-history window to the interviewer, and render the linked append-only messages as one continuous transcript. Keep the completed parent immutable and require the child cycle to become ready plus receive explicit update consent before another replacement. Test fresh-snapshot context, transcript continuity, repeated refinement, and session ownership.
...
- Resume, retention, retry, timeout, and spam settings as Filament fields on Operational settings. The per-provider AI timeout defaults to 180 seconds; the documented two-entry worker and database retry windows are 400 and 415 seconds so queue reservations cannot overlap an active provider chain.
